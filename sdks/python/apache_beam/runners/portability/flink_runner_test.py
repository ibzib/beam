...
class FlinkRunnerTest(portable_runner_test.PortableRunnerTest):
  ...
  def parse_options(self, request):
    ...
    if known_args.flink_job_server_jar:
      self.set_flink_job_server_jar(known_args.flink_job_server_jar)
    else:
      try:
        ...
        pytest.skip(str(e))
    self.streaming = known_args.streaming
    self.environment_type = known_args.environment_type
    self.environment_config = known_args.environment_config
    self.extra_experiments = known_args.extra_experiments
  ...
  def create_options(self):
    ...
    options.view_as(
        DebugOptions).experiments = ['beam_fn_api'] + self.extra_experiments
    options._all_options['parallelism'] = 2
    options.view_as(PortableOptions).environment_type = (
        self.environment_type.upper())
    if self.environment_config:
      options.view_as(
          PortableOptions).environment_config = self.environment_config
